...
use crate::{slotmap::slotmap::{Slotmap, SlotKey}, render_system::render_texture::RenderTexture};
...
impl<E:ExecutableComponent> Deref for ExecutableSlotmap<E> {
    ...
    fn deref(&self) -> &Slotmap<E> { &self.0 }
}

impl<E:ExecutableComponent> DerefMut for ExecutableSlotmap<E> {
    fn deref_mut(&mut self) -> &mut Slotmap<E> { &mut self.0 }
}
...
pub enum EngineDataSlotmapTypes{
    RenderTexture(Slotmap<RenderTexture>)
}
